...
from_json = ak._v2.operations.convert.from_json
from_iter = ak._v2.operations.convert.from_iter
to_json = ak._v2.operations.convert.to_json
to_numpy = ak._v2.operations.convert.to_numpy


def test_from_iter():
    assert from_iter([1 + 1j, 2 + 2j, 3 + 3j]).tolist() == [
        1 + 1j,
        2 + 2j,
        3 + 3j,
    ]
    ...
    assert from_iter([1, 2.2, 3 + 3j]).tolist() == [
        1.0 + 0j,
        2.2 + 0j,
        3.0 + 3j,
    ]
    assert from_iter([1, 3 + 3j]).tolist() == [
        1.0 + 0j,
        3.0 + 3j,
    ]
    ...
    # into reals:
    assert str(from_iter([1, 2.2]).type) == "2 * float64"
    assert from_iter([1, 2.2]).tolist() == [1.0, 2.2]
...
def test_from_json():
    array = from_json('[{"r": 1.1, "i": 1.0}, {"r": 2.2, "i": 2.0}]')
    assert array.tolist() == [
        {"r": 1.1, "i": 1.0},
        {"r": 2.2, "i": 2.0},
    ]
...
def test_reducers():
    assert (
        ak._v2.operations.reducers.sum(from_iter([[1 + 1j, 2 + 2j], [], [3 + 3j]]))
        == 6 + 6j
    )
    assert (
        ak._v2.operations.reducers.prod(from_iter([[1 + 1j, 2 + 2j], [], [3 + 3j]]))
        == -12 + 12j
    )

    assert ak._v2.operations.reducers.sum(
        from_iter([[1 + 1j, 2 + 2j], [], [3 + 3j]]), axis=1
    ).tolist() == [
        3 + 3j,
        0 + 0j,
        3 + 3j,
...
def test_minmax():
    assert (
        ak._v2.operations.reducers.min(from_iter([[1 + 5j, 2 + 4j], [], [3 + 3j]]))
        == 1 + 5j
    )
    assert (
        ak._v2.operations.reducers.max(from_iter([[1 + 5j, 2 + 4j], [], [3 + 3j]]))
        == 3 + 3j
    )

    assert ak._v2.operations.reducers.min(
        from_iter([[1 + 5j, 2 + 4j], [], [3 + 3j]]), axis=1
    ).tolist() == [
        1 + 5j,
        None,
        3 + 3j,
...
def test_numpy():
    assert np.array_equal(
        to_numpy(from_iter([[1 + 1j, 2 + 2j], [3 + 3j, 4 + 4j]])),
        np.array([[1 + 1j, 2 + 2j], [3 + 3j, 4 + 4j]]),
    )
    assert (
        str(to_numpy(from_iter([[1 + 1j, 2 + 2j], [3 + 3j, 4 + 4j]])).dtype)
        == "complex128"
    )
